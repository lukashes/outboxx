...
test "E2E: a same-named table in another schema is not routed to the topic" {
    const allocator = testing.allocator;
    ...
    const timestamp = test_helpers.nowSeconds(std.testing.io);
    const table_name = try std.fmt.allocPrint(allocator, "users_xschema_{d}", .{timestamp});
    defer allocator.free(table_name);
    const other_schema = try std.fmt.allocPrint(allocator, "other_{d}", .{timestamp});
    defer allocator.free(other_schema);
    const topic_name = try std.fmt.allocPrint(allocator, "topic.stream.xschema.{d}", .{timestamp});
    defer allocator.free(topic_name);
    const slot_name = try std.fmt.allocPrint(allocator, "e2e_xschema_slot_{d}", .{timestamp});
    defer allocator.free(slot_name);
    const pub_name = try std.fmt.allocPrint(allocator, "e2e_xschema_pub_{d}", .{timestamp});
    defer allocator.free(pub_name);

    // public.<table>: the one the stream targets.
    try test_helpers.createTestTable(conn, allocator, table_name);

    // A second schema with a table of the SAME name. The publication is FOR ALL
    // TABLES, so its changes reach the decoder and must be dropped by stream
    // matching, not leaked into the public.<table> topic.
    const create_schema = try test_helpers.formatSqlZ(allocator, "CREATE SCHEMA {s};", .{other_schema});
    defer allocator.free(create_schema);
    _ = c.PQexec(conn, create_schema.ptr);
    defer {
        const drop_schema = std.fmt.allocPrint(allocator, "DROP SCHEMA IF EXISTS {s} CASCADE;", .{other_schema}) catch unreachable;
        defer allocator.free(drop_schema);
        const drop_schema_z = allocator.dupeZ(u8, drop_schema) catch unreachable;
        defer allocator.free(drop_schema_z);
        _ = c.PQexec(conn, drop_schema_z.ptr);
    }

    const create_other = try test_helpers.formatSqlZ(allocator,
        \\CREATE TABLE {s}.{s} (id SERIAL PRIMARY KEY, name TEXT NOT NULL, value INT);
        \\ALTER TABLE {s}.{s} REPLICA IDENTITY FULL;
    , .{ other_schema, table_name, other_schema, table_name });
    defer allocator.free(create_other);
    _ = c.PQexec(conn, create_other.ptr);
    ...
    std.debug.print("\n=== E2E CROSS-SCHEMA TEST ===\n", .{});

    // One row into each table; only the public row should be routed.
    const insert_public = try test_helpers.formatSqlZ(allocator, "INSERT INTO {s} (name, value) VALUES ('PublicRow', 1);", .{table_name});
    defer allocator.free(insert_public);
    const insert_other = try test_helpers.formatSqlZ(allocator, "INSERT INTO {s}.{s} (name, value) VALUES ('OtherRow', 2);", .{ other_schema, table_name });
    defer allocator.free(insert_other);
    _ = c.PQexec(conn, insert_public.ptr);
    _ = c.PQexec(conn, insert_other.ptr);
    test_helpers.sleepNs(std.testing.io, 200_000_000); // 200ms
    ...
    // Exactly the public row: the other schema's change must not leak.
    try testing.expectEqual(@as(usize, 1), messages.len);
    try test_helpers.assertJsonField(messages[0], "meta.schema", "public");
    try test_helpers.assertJsonField(messages[0], "meta.resource", table_name);
    try test_helpers.assertJsonField(messages[0], "data.name", "PublicRow");

    std.debug.print("=== TEST COMPLETED SUCCESSFULLY ===\n", .{});
    std.debug.print("✓ Only public.{s} was routed; other schema was dropped\n", .{table_name});
}
