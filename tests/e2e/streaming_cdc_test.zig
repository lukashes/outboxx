...
// E2E Test: Streaming CDC Pipeline Verification
// Tests the complete flow: PostgreSQL → Streaming Replication → Parser → Serializer → Kafka
//
// Uses PostgresStreamingSource (logical replication with pgoutput)
// instead of PostgresPollingSource (pg_logical_slot_peek_changes)
//
...
test "E2E Streaming: INSERT operation - full pipeline verification" {
    ...
    if (c.PQstatus(conn) != c.CONNECTION_OK) {
        std.debug.print("PostgreSQL not available, skipping E2E streaming test\n", .{});
    ...
    // Create test table with REPLICA IDENTITY FULL (required for streaming)
    ...
    // Connect to PostgreSQL with streaming replication
    ...
    processor.initialize() catch |err| {
        std.debug.print("Kafka unavailable, skipping E2E streaming test: {}\n", .{err});
        return error.SkipZigTest;
    };

    std.debug.print("\n=== E2E STREAMING INSERT TEST ===\n", .{});
    ...
    std.Thread.sleep(200_000_000); // 200ms for streaming to catch up

    // Process CDC pipeline
    std.debug.print("Step 2: Process CDC pipeline (streaming mode)\n", .{});
    ...
    std.debug.print("✓ Streaming replication protocol works correctly\n", .{});
}

test "E2E Streaming: UPDATE operation - full pipeline verification" {
    const allocator = testing.allocator;
    ...
    std.debug.print("✓ Streaming replication captured all changes\n", .{});
}

test "E2E Streaming: DELETE operation - full pipeline verification" {
    const allocator = testing.allocator;
