...
pub fn convert(allocator: std.mem.Allocator, oid: u32, text: []const u8) !FieldValue {
...
test "convert: integer types become JSON integers" {
    const allocator = testing.allocator;
    for ([_]u32{ 21, 23, 20 }) |oid| {
        const v = try convert(allocator, oid, "42");
        try testing.expect(v == .integer);
        try testing.expectEqual(@as(i64, 42), v.integer);
    }

    const neg = try convert(allocator, 20, "-9223372036854775808");
    try testing.expectEqual(@as(i64, std.math.minInt(i64)), neg.integer);
}

test "convert: float types become JSON floats" {
    const allocator = testing.allocator;
    for ([_]u32{ 700, 701 }) |oid| {
        const v = try convert(allocator, oid, "3.5");
        try testing.expect(v == .float);
        try testing.expectEqual(@as(f64, 3.5), v.float);
    }
}

test "convert: non-finite floats fall back to string" {
    const allocator = testing.allocator;
    for ([_]u32{ 700, 701 }) |oid| {
        for ([_][]const u8{ "NaN", "Infinity", "-Infinity" }) |text| {
            const v = try convert(allocator, oid, text);
...
test "convert: bool maps t/f to JSON boolean" {
    const allocator = testing.allocator;
    const t = try convert(allocator, 16, "t");
    try testing.expect(t == .bool and t.bool == true);
    const f = try convert(allocator, 16, "f");
    try testing.expect(f == .bool and f.bool == false);
}

test "convert: numeric stays a string" {
    const allocator = testing.allocator;
    const v = try convert(allocator, 1700, "12345.6789");
    defer allocator.free(v.string);
    try testing.expect(v == .string);
    try testing.expectEqualStrings("12345.6789", v.string);
}

test "convert: unknown OID stays a string" {
    const allocator = testing.allocator;
    // 25 = text
    const v = try convert(allocator, 25, "hello");
    defer allocator.free(v.string);
    try testing.expect(v == .string);
    try testing.expectEqualStrings("hello", v.string);
}

test "convert: unparseable integer falls back to string" {
    const allocator = testing.allocator;
    const v = try convert(allocator, 23, "not-a-number");
    defer allocator.free(v.string);
    try testing.expect(v == .string);
    try testing.expectEqualStrings("not-a-number", v.string);
}

test "convert: unparseable float falls back to string" {
    const allocator = testing.allocator;
    const v = try convert(allocator, 701, "not-a-float");
    defer allocator.free(v.string);
    try testing.expect(v == .string);
